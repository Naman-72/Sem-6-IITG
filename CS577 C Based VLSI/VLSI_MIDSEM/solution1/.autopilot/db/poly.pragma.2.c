...
# 4 "dilithium3/poly.c" 2
# 1 "dilithium3/ntt.h" 1
...
# 5 "dilithium3/poly.c" 2
# 1 "dilithium3/reduce.h" 1
# 11 "dilithium3/reduce.h"
...
# 7 "dilithium3/poly.c" 2
# 1 "dilithium3/symmetric.h" 1
# 35 "dilithium3/symmetric.h"
# 1 "dilithium3/fips202.h" 1



# 1 "D:/Xilinx/Vivado/2019.2/win64/tools/clang/bin/../lib/clang/3.1/include\\stddef.h" 1 3 4
# 5 "dilithium3/fips202.h" 2
# 15 "dilithium3/fips202.h"
typedef struct {
  uint64_t s[25];
  unsigned int pos;
} keccak_state;
...
void pqcrystals_dilithium3_ref_dilithium_shake256_stream_init(keccak_state *state,
                                    const uint8_t seed[48],
                                    uint16_t nonce);
# 8 "dilithium3/poly.c" 2
# 28 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_poly_reduce(poly *a) {
  unsigned int i;
                ;
...
{_ssdm_SpecArrayDimSize(seed, 32);
  ...
  pqcrystals_dilithium3_ref_dilithium_shake128_stream_init(&state, seed, nonce);
  shake128_squeezeblocks(buf, ((227 + 168 - 1)/168), &state);
...
{_ssdm_SpecArrayDimSize(seed, 48);
  ...
  shake256_squeezeblocks(buf, ((640 + 136 - 1)/136), &state);
  pqcrystals_dilithium3_ref_polyz_unpack(a, buf);
}
# 511 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_poly_challenge(poly *c, const uint8_t seed[32]) {_ssdm_SpecArrayDimSize(seed, 32);
...
# 554 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_polyeta_pack(uint8_t *r, const poly *a) {
  unsigned int i;
  uint8_t t[8];
                ;
# 575 "dilithium3/poly.c"
  for(i = 0; i < 256/2; ++i) {
...
# 593 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_polyeta_unpack(poly *r, const uint8_t *a) {
  unsigned int i;
                ;
# 618 "dilithium3/poly.c"
  for(i = 0; i < 256/2; ++i) {
...
# 798 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_polyz_pack(uint8_t *r, const poly *a) {
  unsigned int i;
  uint32_t t[4];
                ;
# 824 "dilithium3/poly.c"
  for(i = 0; i < 256/2; ++i) {
...
# 849 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_polyz_unpack(poly *r, const uint8_t *a) {
  unsigned int i;
                ;
# 881 "dilithium3/poly.c"
  for(i = 0; i < 256/2; ++i) {
...
# 910 "dilithium3/poly.c"
void pqcrystals_dilithium3_ref_polyw1_pack(uint8_t *r, const poly *a) {
  unsigned int i;
                ;
# 924 "dilithium3/poly.c"
  for(i = 0; i < 256/2; ++i)
