...
ENVIRONMENT: LINUX g++ 
gcc version : 9.3.0
Command To compile the source code : g++ A2_200101111.cpp
        ...
        if (ifcomment == false)
        {
            if (LABEL != "")
            {
                ...
                else
                {
                    ...
                    SYMTAB[LABEL] = LOCCTR;
                    writeData = LABEL + "\t" + LOCCTR;
                    writeToFile(symbolFile,writeData);
                }
            }
            writeIntermediateLine(LOCCTR, &fout);
    ...
    while (OPCODE != "END")
    {
        if (ifcomment == false)
        {
            if (OPTAB.find(OPCODE) != OPTAB.end())
            {
                ...
                writeData = OPCODE + "\t"+ OPTAB.at(OPCODE);
                writeToFile(opcode,writeData);
            }
